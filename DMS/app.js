...
const doctName = document.getElementById("doctorName")
...
async function displayPrescriptionDetails() {
    ...
    if (prescriptionDetails) {
        ...
        document.body.innerHTML = "You cannot access this page directly!"
    }
...
async function updateAppointmentsTable() {
    try {
        ...
        let appointments = await getDoctorAppointments();
...
if (!doctor && currentPageName !== "login.html") {
    window.location.pathname = "login.html";

} else if (doctor && doctName && currentPageName !== "login.html") {
    console.log(doctor);
    doctName.innerHTML = 'Dr. ' + doctor[0].DoctorName;
    if (currentPageName === "index.html") {
        async function handleAppointments() {
            try {
                let patInfo = await getDoctorAppointments();
                if (!patInfo) {
                    patInfo = [];
                    patInfo.length = 0;
                }
                let pastAppointments = await getPastAppointments();
                const contactInfo = document.getElementsByClassName("contact-name");
                ...
                if (!pastAppointments) {
                    ...
                    contactInfo1[1].innerText = " " + pastAppointments[1].Email;
                } else if (pastAppointments && pastAppointments.length === 1) {
                    contactInfo[0].innerText = pastAppointments[0].PatientName;
                    contactInfo1[0].innerText = " " + pastAppointments[0].Email;
                    contactInfo[1].innerText = "";
...
else if (doctor && currentPageName === "login.html") {
    window.location.pathname = "index.html";
}
    ...
    if (billData) {
        ...
        document.body.innerHTML = '<h1>Error! You cannot access this page directly!</h1>';
